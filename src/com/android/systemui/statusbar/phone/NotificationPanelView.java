...
public class NotificationPanelView extends PanelView implements
    ...
    private static String TAG = NotificationPanelView.class.getSimpleName();
   ...
   // private int mScrollYOverride = -1;
    ...
    protected void onFinishInflate() {
        ...
        //add by pengchong
       // mScrollView = (ObservableScrollView) findViewById(R.id.observalblescrollview);
    ...
    public void updateResources() {
        ...
        Log.d(TAG,"updateResource lp = "+lp.height);
...
//        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mQs != null) {
           ...
           // mQsMinExpansionHeight = 0;
            ...
            Log.d(TAG,"getDessiredHeight1111 = "+mQs.getDesiredHeight());
        }
        positionClockAndNotifications();
        if (mQsExpanded && mQsFullyExpanded) {
            mQsExpansionHeight = mQsMaxExpansionHeight;
            Log.d(TAG,"onLayout mQsExpansionHeight = "+mQsExpansionHeight);
        ...
        if (mQsSizeChangeAnimator == null && mQs != null) {
            Log.d(TAG,"mQsSizeChangeAnimator == null onLayout mQs.getDesiredHeight() setHeightOverride= "+mQs.getDesiredHeight());
    ...
    private void startQsSizeChangeAnimation(int oldHeight, final int newHeight) {
        Log.d(TAG,"startQsSizeChangeAnimation oldHeight = "+oldHeight+" newHeight = "+newHeight);
        ...
        mQsSizeChangeAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                Log.d(TAG,"mQsSizeChangeAnimator setHeightOverride  height = "+height);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d("PanelBar","mBlockTouches = "+mBlockTouches+" (mQs != null && mQs.isCustomizing()) = "+(mQs != null && mQs.isCustomizing()));
         if (mBlockTouches || (mQs != null && mQs.isCustomizing())) {
             Log.d("PanelBar","onTouchEvent");
            return false;
        }
        ...
        if (mOnlyAffordanceInThisMotion) {
            Log.d("PanelBar","mOnlyAffordanceInThisMotion = "+mOnlyAffordanceInThisMotion);
            return true;
        }
        handled |= mHeadsUpTouchHelper.onTouchEvent(event);

        if (!mHeadsUpTouchHelper.isTrackingHeadsUp() && handleQsTouch(event)) {
            Log.d("PanelBar","mHeadsUpTouchHelper.isTrackingHeadsUp() = "+mHeadsUpTouchHelper.isTrackingHeadsUp()+" handleQsTouch(event) = "+handleQsTouch(event));
            return true;
        }
...
//        mScrollView.scrollTo(0, 0);
        ...
        Log.d(TAG,"onQsExpandsionStarted overscrollAmount = "+height);
        setQsExpansion(height);
        requestPanelHeightUpdate();
    ...
    private void setQsExpansion(float height) {
        ...
        Log.d(TAG,"setQsExpansion height1 = "+height);
        ...
        Log.d(TAG,"setQsExpanHeight = "+mQsExpansionHeight);
        updateQsExpansion();
        requestScrollerTopPaddingUpdate(false /* animate */);
        if (mKeyguardShowing) {
            updateHeaderKeyguardAlpha();
    ...
    private float calculateQsTopPadding() {
        if (mKeyguardShowing
            ...
            int max = mStatusBarState == StatusBarState.KEYGUARD
                    ...
                    : maxQs;
            Log.d(TAG,"111111111 = "+(int) interpolate(getExpandedFraction(),
                    mQsMinExpansionHeight, max));
            return (int) interpolate(getExpandedFraction(),
                    mQsMinExpansionHeight, max);
        } else if (mQsSizeChangeAnimator != null) {
            Log.d(TAG,"222222222222getAnimated  = "+(int) mQsSizeChangeAnimator.getAnimatedValue());
            return (int) mQsSizeChangeAnimator.getAnimatedValue();
        } else if (mKeyguardShowing) {

            // We can only do the smoother transition on Keyguard when we also are not collapsing
            // from a scrolled quick settings.
            Log.d(TAG,"333333333333 = "+ interpolate(getQsExpansionFraction(),
            ...
            Log.d(TAG,"4444444444444mQsExpansionHeight = "+mQsExpansionHeight);
            return mQsExpansionHeight;
        }
    }

    protected void requestScrollerTopPaddingUpdate(boolean animate) {
        Log.d(TAG,"requestScrollerTopPaddingUpdate animate  = "+animate);
    ...
    protected int getMaxPanelHeight() {
        ...
        int maxHeight;
        Log.d(TAG, "mQsExpandImmediate = "+mQsExpandImmediate+" mQsExpanded = "+mQsExpanded+" mIsExpanding = "+mIsExpanding+" mQsExpandedWhenExpandingStarted = "+mQsExpandedWhenExpandingStarted);
        ...
        Log.d(TAG,"getMaxPanelHeight maxHeight = "+maxHeight+" min = "+min);
        maxHeight = Math.max(maxHeight, min);
        Log.d(TAG,"getMaxPanelHeight maxHeight = "+maxHeight);
    ...
    protected void onHeightUpdated(float expandedHeight) {
        Log.d(TAG,"onHeightUpdated expandedHeight = "+expandedHeight);
        ...
        if (mQsExpandImmediate || mQsExpanded && !mQsTracking && mQsExpansionAnimator == null
            ...
            Log.d(TAG,"onHeightUpdated = "+ mQsMinExpansionHeight
                    + t * (getTempQsMaxExpansion() - mQsMinExpansionHeight));
        }
...
//        return qsTempMaxExpansion;
    ...
    private int calculatePanelHeightShade() {
        ...
        Log.d(TAG,"maxHeight = "+maxHeight+" height = "+mNotificationStackScroller.getHeight()+" mTopPaddingAdjustment = "+mTopPaddingAdjustment);
        maxHeight += mNotificationStackScroller.getTopPaddingOverflow();
        Log.d(TAG," mNotificationStackScroller.getTopPaddingOverflow() = "+mNotificationStackScroller.getTopPaddingOverflow());
    ...
    public void onQsHeightChanged() {
        Log.d(TAG,"onQsHeightChanged = "+mQs.getDesiredHeight());
        mQsMaxExpansionHeight = mQs != null ? mQs.getDesiredHeight() : 0;
        if (mQsExpanded && mQsFullyExpanded) {
            mQsExpansionHeight = mQsMaxExpansionHeight;
            Log.d(TAG,"onQsHeightChanged mQsExpansionHeight = "+mQsExpansionHeight);
            requestScrollerTopPaddingUpdate(false /* animate */);
            requestPanelHeightUpdate();
        }
    }
    ...
    public void onClick(View v) {
        Log.d(TAG,"NotificationiPanelView onClick");
...
//    }
    ...
    @Override
    public void onOverscrolled(float lastX, float lastY, int amount) {
        if (mIntercepting && shouldQuickSettingsIntercept(lastX, lastY,
